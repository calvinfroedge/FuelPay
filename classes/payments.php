...
{
	/**
	 * The mode the payments class is in.  Will be either "test" or "production" depending on your config settings.
	*/
	public static $mode;
	...
	*/
	...
	/**
	 * The init function (similar to a constructor)
	*/
	...
	*/ 	
	public static function __callStatic($method, $params)
	{
		...
		if($supported)
		{
			$response = self::_make_gateway_call($params[0], $method, $params[1]);
		}
		else
		{
	...
	/**
	* Checks to ensure that a method is actually supported by payments class before continuing
	*
	* @param string The payment method to use
	* @return object Should return a success or failure, along with a response.
	*/
	private static function _check_method_supported($method)
	{
		$supported_methods = \Config::get('payments.supported_methods');
	...
	* @return object Should return a success or failure, along with a response.
	*/
	private static function _make_gateway_call($payment_module, $payment_type, $params)
	{
		$module_exists = self::_load_module($payment_module);
		...
		{
			\Config::load('payments/'.$payment_module);
			self::$payment_type = $payment_type;	
			$valid_inputs = self::_check_inputs($payment_module, $params);
			if($valid_inputs === TRUE)
			{
				self::$_params = $params;	
				$response = self::_do_method($payment_module);
				return $response;		
			}
			else
			{
	 ...
	 */		
	private static function _load_module($payment_module)
	{
	 ...
	 * @return	mixed	Will return bool if file is not found.  Will return file as object if found.
	 */
	private static function _check_inputs($payment_module, $params)
	{
		...
		$expected_datatypes = self::_check_datatypes($expected_datatypes);
		...
		$expected_params = self::_check_params($payment_module, $params);
	 ...
	 */		
	private static function _check_datatypes($datatypes)
	{
		$invalids = 0;
	 ...
	 */			
	private static function _check_params($payment_module, $params)
	{
		//Ensure required params are present
		$req = \Config::get('required_params');
		if( ! isset($req[self::$payment_type]))
		{
			return self::return_response(
				'failure', 
				'not_a_method', 
		...
		self::$required_params = $req;
		$req = $req[self::$payment_type];
		...
		{	
			...
			{
				$missing[] = \Lang::line('response_detail_missing_'.$v);
			}
		}
		...
		if(array_key_exists('cc_exp', $params))
		{
			...
			if(strlen($exp_date) != 6 OR !is_numeric($exp_date) OR $m1 > 1)
			{
				return self::return_response(
					...
					'invalid_input', 
					'local_response', 
					\Lang::line('response_detail_invalid_date_format')
				);
			}
		}
		...
		if(array_key_exists('billing_period', $params))
		{
			...
			if(!in_array($params['billing_period'], $accepted_billing_period))
			{
				return self::return_response(
					...
					'invalid_input', 
					'local_response', 
					\Lang::line('response_detail_invalid_billing_period')
				);			
			}
		}
	...
	*/	
	public static function gateway_request($query_string, $payload = NULL, $content_type = NULL)
	{
		...
		else
		{
			...
			$request = $this->ci->curl->execute();
			$parsed = self::parse_xml($request);	
			return $parsed;	
		}
		*/
		exit('calls to gateway not yet implemented...need a curl class!');
	}
	...
	*/	
	public static function return_response($status, $response, $response_type, $response_details = null)
	{
		...
		{
			$local_response = self::_return_local_response($status, $response, $response_details);
			return $local_response;
		}
		else if($response_type == 'gateway_response')
		{
			$gateway_response = self::_return_gateway_response($status, $response, $response_details);
	 ...
	 * @return	object	
	*/	
	private static function _return_local_response($status, $response, $response_details = null)
	{
		$status = strtolower($status);
	 ...
	 * @return	object	
	*/	
	private static function _return_gateway_response($status, $response, $details)
	{	
		return (object) array
		(
	 ...
	 * @return	bool
	 */
	private static function _connection_is_secure()
	{
		// Check whether secure connection is required
		if(\Config::get('payments.force_secure_connection') === FALSE) 
		{
			Log::info('WARNING!! Using Payment Gateway without Secure Connection!');
			return FALSE;
		}
